...
def run_inverse_analysis(analysis_type,
    ...
    model_mixed_angular_drivers = int(count_model_DoF(modeling_file)['mixed_dofs'])
    ...
    if analysis_type.lower() == 'inverse dynamic':
        ...
        moments_of_force_rep = np.zeros((nFrames, angDriversConstraints + model_mixed_angular_drivers))
    ...
    for frame in range(0, nFrames):
        ...
        if analysis_type.lower() == 'inverse dynamic':
            # if model_mixed_angular_drivers > 1:
            ...
            #     cols_to_remove = [x for x in range(nRigidBodies * 4, nRigidBodies * 4 + model_mixed_angular_drivers)]
            #
            ...
            #
            # A = dPhidq_updated.T
            # b = generalized_forces_vector[: nRigidBodies * 4] - massMatrix[: nRigidBodies * 4, : nRigidBodies * 4].dot(qpp[: nRigidBodies * 4])
            #
