...
def compute_spring_damper_actuator_force(q, qp, sda_parameters):
    ...
    Function computes the spring damper actuatur (SDA) forces associated with
    ...
    if spring_length < spring_lo:
        force1 = applyForce(sdaForce, locCoords1)
        force2 = -applyForce(sdaForce, locCoords2)
    else:
        force1 = -applyForce(sdaForce, locCoords1)
        force2 = applyForce(sdaForce, locCoords2)
