...
def read_muscle_db(muscle_db_path, scaling_factor, rb_info, muscle_specific_tension, verbose):
    ...
    Function reads and scales the local coordaintes of the via points of each muscle.
    ...
    for row in muscle_db_data:
        ...
        if int(muscle_on_off) == 1:
            ...
            muscle_element_info = {'muscle':muscle_name,
                                   'on_off': int(muscle_on_off),  # muscle on (ative) or off (inactive)
                                   'type': muscle_type,
                                   'element': muscle_element,
                                   'origin':{'body': rb_info[muscle_Origin_Nb],                                                     # Body number
                                                   'coords': [ float(muscle_origin_LocCoord_X) * scaling_factor,                    # AP coordinate
                                                               # float(muscle_origin_LocCoord_Y),                                   # ML coordinate
                                                               float(muscle_origin_LocCoord_Z) * scaling_factor]},                  # Long coordinate
                                                   'insertion':{'body': rb_info[muscle_insertion_Nb],                               # Body number
                                                                'coords': [float(muscle_insertion_LocCoord_X * scaling_factor),     # AP coordinate
                                                                 # float(muscle_insertion_LocCoord_Y),                              # ML coordinate
                                                                 float(muscle_insertion_LocCoord_Z) * scaling_factor]},             # Long coordinate
                                   'fo': float(muscle_PCSA * muscle_specific_tension),                                              # muscle MVC force
                                   'alpha': float(muscle_pennation_angle),                                                          # muscle_pennation_angle
                                   'lm': float(muscle_opt_length) ,                                                                  # muscle_resting_length
                                   'lt': float(tendon_slack_length) ,                                                                      # tendon_slack_length
                                   'n_via_points': muscle_n_via_points                                                              # Muscle_n_via_points
                                   }
            ...
            if str(muscle_type).lower().strip() == 'vp':
                ...
                for vp_number in range(0, muscle_n_via_points):
                    ...
                    vp_info['vp' + str(vp_number + 1)] = {'body': rb_info[row[start_idx + 4].lower()],              # Body number
                                                          'coords': [float(row[start_idx + 1]) * scaling_factor,    # AP coordinate
                                                                     # float(row[start_idx + 2]) * scaling_factor,  # ML coordinate
                                                                     float(row[start_idx + 3]) * scaling_factor]}   # Long coordinate
